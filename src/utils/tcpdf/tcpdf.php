...
class TCPDF {
	...
	public function addHtmlLink($url, $name, $fill=false, $firstline=false, $color=null, $style=-1, $firstblock=false) {
		if (isset($url[1]) AND ($url[0] == '#') AND is_numeric($url[1])) {
			// Check if this is a pre-registered internal link ID (from AddLink)
			$numericId = intval(substr($url, 1));
			if (isset($this->links[$numericId]) AND (strpos($url, ',') === false)) {
				// Use the pre-registered link directly (SetLink will set destination later)
				$url = $numericId;
			} else {
				// convert url to internal link (page,y format)
				$lnkdata = explode(',', $url);
				if (isset($lnkdata[0]) ) {
			...
			case 'img': {
				...
				if (isset($this->HREF['url']) AND !TCPDF_STATIC::empty_string($this->HREF['url'])) {
					...
					if ($imglink[0] == '#' AND isset($imglink[1]) AND is_numeric($imglink[1])) {
						// Check if this is a pre-registered internal link ID
						$numericId = intval(substr($imglink, 1));
						if (isset($this->links[$numericId]) AND (strpos($imglink, ',') === false)) {
							$imglink = $numericId;
						} else {
