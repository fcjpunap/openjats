<?php
/**
 * Clase ScieloGenerator - Generación de XML-JATS con estándares de SciELO (SPS 1.9)
 */
...
class ScieloGenerator {
    ...
    private function createArticleMeta($dom, $article, $authors, $affiliations) {
        ...
        $sg->appendChild($dom->createElement('subject', htmlspecialchars($article['article_type'] ?? 'Articulo')));
        ...
        $lic = $dom->createElement('license'); $lic->setAttribute('license-type', 'open-access'); $lic->setAttribute('xlink:href', 'http://creativecommons.org/licenses/by/4.0/'); $lic->setAttribute('xml:lang', 'es');
