...
class JATSPDF extends TCPDF {
    ...
    public function Header() {
        ...
        if (!file_exists($logoLeft)) $logoLeft = 'https://fcjp.derecho.unap.edu.pe/catg/jats-assistant/public/journal.jpeg';
        $logoRight = realpath(__DIR__ . '/../../public/logofcjp.png');
        if (!file_exists($logoRight)) $logoRight = 'https://fcjp.derecho.unap.edu.pe/catg/jats-assistant/public/logofcjp.png';
...
class PDFGenerator {
    ...
    public function generatePDF($articleId) {
        ...
        // Fechas
        $datesText = [];
        if (!empty($article['received_date'])) $datesText[] = "Recibido / Received: " . date('d/m/Y', strtotime($article['received_date']));
        if (!empty($article['accepted_date'])) $datesText[] = "Aceptado / Accepted: " . date('d/m/Y', strtotime($article['accepted_date']));
        if (!empty($article['published_date'])) $datesText[] = "Publicado / Published: " . date('d/m/Y', strtotime($article['published_date']));
        ...
        if (!empty($rm[1])) $refTargetsInText = array_values(array_unique($rm[1]));
        preg_match_all('/href=[\'"]#(fn-[^\'"]+)[\'"]/i', $allContent, $fm);
        if (!empty($fm[1])) $fnTargetsInText = array_values(array_unique($fm[1]));
        ...
        foreach ($refTargetsInText as $t) {
            if (!isset($nativeLinks[$t])) $nativeLinks[$t] = $pdf->AddLink();
        }
        if (!empty($references)) {
        ...
        foreach ($sections as $section) {
            ...
            for ($pass = 0; $pass < 5; $pass++) {
                if (!preg_match('/<a\s[^>]*>[^<]*<a\s/i', $content) && 
                    !preg_match('/<a\s[^>]*>\s*<sup>\s*<a\s/i', $content)) break;
        ...
        foreach ($figures as $f) {
            if (in_array(trim($f['label'] ?? ''), $this->usedFigureLabels)) continue;
